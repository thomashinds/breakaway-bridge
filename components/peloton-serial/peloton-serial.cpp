...
PelotonSerial::PelotonSerial(EventHandler &event_handler) : event_handler{event_handler} {
    ...
    xTaskCreateStatic(this->Task, "PelotonSerial", this->STACK_SIZE, this, tskIDLE_PRIORITY + 4,
...
void PelotonSerial::Task(void *context) {
    ...
    uint8_t *data = (uint8_t *)malloc(1024);  // todo make static and smaller
    ...
    while (1) {
        ...
        int len = uart_read_bytes(2, data, 1024, pdMS_TO_TICKS(20));
        if (len > 0) {
            uint8_t dir = data[0];
